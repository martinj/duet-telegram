...
module.exports = (config, duet) => {
	...
	bot.hears(/\/gcode (.*)$/, commands.gcode);
        ...
        bot.hears(/\/speedfactor (\d{2,3})$/, commands.speedfactor);
        bot.hears(/\/extrfactor (\d{2,3})$/, commands.extrfactor);
        bot.hears(/\/babystep ([-|\+]?\d{1,2})$/, commands.babystep);
	// commands
	...
	bot.command('notifications', commands.notifications);
        ...
        bot.command('speedfactor', commands.speedfactor);
        bot.command('extrfactor', commands.extrfactor);
        bot.command('babystep', commands.babystep);
	...
	bot.action('printfile', commands.printFile);
        ...
        bot.action('msgok', commands.msgOK);
        bot.action('msgcancel', commands.msgCancel);
