...
exports.document = async (ctx) => {
	...
	return ctx.reply(...fileBrowser.createPrintDialog(filePath));
};

exports.printDialog = (ctx) => {
	const file = ctx.match[1];
	const back = Markup.callbackButton('↩️ Back', 'listfiles /gcodes 1');
	const dialog = fileBrowser.createPrintDialog(file, [back]);
...
exports.printFile = async (ctx) => {
	const file = ctx.match[1].replace(/\/gcodes\//, '');
