...
exports.document = async (ctx) => {
	...
	try {
		await ctx.duet.upload(url, `/gcodes/${ctx.config.uploadDir}/${doc.file_name}`);
		ctx.reply('File was successfully uploaded.');
	} catch (err) {
...
exports.files = async (ctx) => {
	...
	fileBrowser.list('/gcodes', 1, 'printfile', res.message_id)(ctx);
};
