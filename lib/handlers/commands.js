...
exports.status = async (ctx) => {
	try {
	  ...
	  const res = await ctx.duet.getStatus(3);
		...
		if (res.status === 'O') {
			verbstat = 'PRINTER OFF';
		} else if (res.status === 'H') {
			verbstat = 'PRINTER HALTED ';
		} else if (res.status === 'D') {
			verbstat = 'PRINTER PAUSING';
		} else if (res.status === 'S') {
			verbstat = 'PRINTER PAUSED';
		} else if (res.status === 'R') {
			verbstat = 'PRINTER RESUMING';
		} else if (res.status === 'M') {
			verbstat = 'PRINTER SIMULATING';
		} else if (res.status === 'B') {
			verbstat = 'PRINTER BUSY';
		} else if (res.status === 'T') {
			verbstat = 'PRINTER TOOL CHG';
		} else if (res.status === 'I') {
			verbstat = 'PRINTER IDLE';
		}
		if (output) {
			//    dmsg = `Message: ${output.msgBox.title}\n${output.msgBox.msg}\n`;
			if (output.msgBox.mode === 3) {
				useraction = 3;
			} else if (output.msgBox.mode === 2) {
				useraction = 2;
			}
		}

		//          let cpos = `Position X=${coords.xyz[0]},Y=${coords.xyz[1]},Z=${coords.xyz[2]}`;
		//	  const returnstr =  `${dmsg} ${verbstat} \n ${cpos}\n Bed ${temps.bed.current}/${temps.bed.active},\n Extruder ${temps.current[currentTool + 1]}/${temps.tools.active[currentTool]},\n Chamber ${temps.chamber.current}/${temps.chamber.active}.`;
		const returnstr = `<b>${verbstat}</b> \n${status.tempStatus(res)}`;
		if (useraction === 3) {
			return ctx.replyWithHTML(returnstr,
				Markup.inlineKeyboard([
					Markup.callbackButton('👍 OK', 'msgok'),
					Markup.callbackButton('🚫 Cancel', 'msgcancel')
				]).extra());
...
exports.msgOK = async (ctx) => {
	const gcode = 'M292 P0';
	await ctx.duet.sendGCode(gcode);
	return ctx.reply('OK response send.');
};

exports.msgCancel = async (ctx) => {
	const gcode = 'M292 P1';
	await ctx.duet.sendGCode(gcode);
	return ctx.reply('CANCEL response send.');
};
...
exports.filsensor = async (ctx) => {
	...
	if (!ctx.match) {
		...
		param = ctx.match[1];
		if (ctx.match[1] === 'on') {
			vgcode = 'M591 D0 S1 M591 D0';
		} else if (ctx.match[1] === 'off') {
			vgcode = 'M591 D0 S0 M591 D0';
		}
	}
...
exports.speedfactor = async (ctx) => {
	let vgcode = 'M220';
	let param = '???';
	if (!ctx.match) {
		vgcode = 'M220';
	} else {
		param = Number(ctx.match[1]);
		vgcode = `M220 S${param}`;
	}
	const gcode = vgcode;
	await ctx.duet.sendGCode(gcode);
...
exports.babystep = async (ctx) => {
	let vgcode = 'M290';
	let param = '???';
	if (!ctx.match) {
		vgcode = 'M290';
	} else {
		param = ctx.match[1] * 0.01;
		vgcode = `M290 S${param}`;
	}
	const gcode = vgcode;
	await ctx.duet.sendGCode(gcode);
...
/speedfactor - set speed factor value in %
/extrfactor - set extrusion factor value
/babystep - change baby step of +/-n*0.01 mm
