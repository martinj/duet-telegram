...
class Poller extends EventEmitter {
	...
	async update() {
		...
		if (this.lastStatus.status === status.status) {
			this.checkForConsoleMessage(status);
			this.lastStatus = status;
			return;
		}
	...
	async statusChanged(status) {
		...
		this.triggerEvent(
			'statusEvent',
			statusEventMap[status.status],
			status,
			status.status === 'P' && this.currentFile
		);
	}
	...
	async checkForConsoleMessage(status) {

	    const {output} = status;
	    if (output) {
	      if (output.msgBox.seq != this.oldmsgseq) {
	        this.oldmsgseq = output.msgBox.seq;
	        const msg = `==== ${output.msgBox.title} ====\n${output.msgBox.msg}`;
	        this.triggerEvent('console', msg, status);
	      }
	    }  
	...
	async startPrintTimer() {
		...
		this.printTimerId = setInterval(() => {
			this.emit('printTime', this.printStarted, this.currentFile, this.lastStatus);
		}, 60000).unref();
	}
